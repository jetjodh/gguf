    ...
    if get_field(reader, 'tokenizer.ggml.model', str) == 't5':
        if temb_shape == (256384, 4096):
            ...
            raise NotImplementedError(
                f'Unknown model detected, rebuild tokenizer failed')
    else:
        raise NotImplementedError(f'Unknown model, rebuild tokenizer failed')
    spm.normalizer_spec.add_dummy_prefix = get_field(reader,
        'tokenizer.ggml.add_space_prefix', bool)
    spm.normalizer_spec.remove_extra_whitespaces = get_field(reader,
        'tokenizer.ggml.remove_extra_whitespaces', bool)
