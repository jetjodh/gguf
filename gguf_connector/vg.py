...
def generate_video(prompt, negative_prompt, width, height, num_frames,
    ...
    video = pipe(prompt=prompt, negative_prompt=negative_prompt, width=
        width, height=height, num_frames=num_frames, num_inference_steps=
        num_inference_steps).frames[0]
...
with block:
    ...
    with gr.Row():
        ...
        quick_prompts.click(lambda x: x[0], inputs=[quick_prompts], outputs
            =prompt_input, show_progress=False, queue=False)
    with gr.Row():
    ...
    generate_btn.click(fn=generate_video, inputs=[prompt_input,
        neg_prompt_input, width_input, height_input, num_frames_input,
        num_steps_input, fps_input], outputs=output_video)
block.launch()