### gguf node for comfyui [![Static Badge](https://img.shields.io/badge/ver-0.1.4-black?logo=github)](https://github.com/calcuis/gguf/releases)
...
`gguf` node is no conflict with the popular `comfyui-gguf` node (can coexist; and this project actually inspired by it; built upon its code base; we are here honor their developers' contribution; then you could test them both; or mixing up use, switch in between freely, all for your own purpose); and is more lightweight (no dependencies needed), more function (i.e., built-in tensor cutter, gguf convertor), compatible with the latest version numpy and other updated libraries come with comfyui
